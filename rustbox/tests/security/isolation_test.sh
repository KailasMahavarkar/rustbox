...
if sudo $MINI_ISOLATE init --box-id 1 >/dev/null 2>&1; then
    if result=$(sudo $MINI_ISOLATE run --box-id 1 --mem 50 --cpu 5 --time 5 /bin/ps -- aux 2>&1); then
    ...
    sudo $MINI_ISOLATE cleanup --box-id 1 >/dev/null 2>&1 || true
else
...
if sudo $MINI_ISOLATE init --box-id 2 >/dev/null 2>&1; then
    if result=$(sudo $MINI_ISOLATE run --box-id 2 --mem 50 --cpu 5 --time 5 /bin/cat -- /etc/passwd 2>&1); then
    ...
    sudo $MINI_ISOLATE cleanup --box-id 2 >/dev/null 2>&1 || true
else
...
if sudo $MINI_ISOLATE init --box-id 3 >/dev/null 2>&1; then
    if result=$(sudo $MINI_ISOLATE run --box-id 3 --mem 50 --cpu 5 --time 5 /bin/ping -- -c 1 8.8.8.8 2>&1); then
    ...
    sudo $MINI_ISOLATE cleanup --box-id 3 >/dev/null 2>&1 || true
else
...
if sudo $MINI_ISOLATE init --box-id 4 >/dev/null 2>&1; then
    if result=$(sudo $MINI_ISOLATE execute-code --box-id 4 --language python --mem 50 --cpu 5 --time 5 --code "import os; print('uid=' + str(os.getuid()) + ' gid=' + str(os.getgid()))" 2>&1); then
        if [[ "$result" == *"\"status\": \"Success\""* ]]; then
    ...
    sudo $MINI_ISOLATE cleanup --box-id 4 >/dev/null 2>&1 || true
else
