...
if sudo $MINI_ISOLATE init --box-id 10 >/dev/null 2>&1; then
    # Ensure box directory exists and copy malicious script into isolate
    sudo mkdir -p "/tmp/rustbox/10" 2>/dev/null || true
    sudo cp "$MALICIOUS_PY" "/tmp/rustbox/10/malicious.py" 2>/dev/null || true
    
    if result=$(sudo $MINI_ISOLATE run --box-id 10 --mem 50 --cpu 5 --time 5 python3 -- malicious.py 2>&1); then
    ...
    sudo $MINI_ISOLATE cleanup --box-id 10 >/dev/null 2>&1 || true
else
...
if sudo $MINI_ISOLATE init --box-id 11 >/dev/null 2>&1; then
    # Try to access external network
    if result=$(sudo $MINI_ISOLATE run --box-id 11 --mem 50 --cpu 5 --time 5 /bin/ping -- -c 1 -W 2 8.8.8.8 2>&1); then
    ...
    sudo $MINI_ISOLATE cleanup --box-id 11 >/dev/null 2>&1 || true
else
...
if sudo $MINI_ISOLATE init --box-id 12 >/dev/null 2>&1; then
    ...
    for attempt in "${escape_attempts[@]}"; do
        if result=$(sudo $MINI_ISOLATE run --box-id 12 --mem 50 --cpu 5 --time 2 $attempt 2>&1); then
    ...
    sudo $MINI_ISOLATE cleanup --box-id 12 >/dev/null 2>&1 || true
else
...
if sudo $MINI_ISOLATE init --box-id 13 >/dev/null 2>&1; then
    if result=$(sudo $MINI_ISOLATE run --box-id 13 --mem 50 --cpu 5 --time 5 /bin/ps -- aux 2>&1); then
        if [[ "$result" == *"Status: Success"* ]]; then
    ...
    sudo $MINI_ISOLATE cleanup --box-id 13 >/dev/null 2>&1 || true
else
...
if sudo $MINI_ISOLATE init --box-id 14 >/dev/null 2>&1; then
    # Try to bypass memory limits with various techniques
    if result=$(sudo $MINI_ISOLATE run --box-id 14 --mem 10 --cpu 2 --time 3 /bin/sh -- -c 'dd if=/dev/zero of=/tmp/bigfile bs=1M count=50 2>/dev/null' 2>&1); then
    ...
    sudo $MINI_ISOLATE cleanup --box-id 14 >/dev/null 2>&1 || true
else
