...
if [ ! -f "../target/release/rustbox" ]; then
    ...
    echo "Using existing rustbox binary..."
fi
...
sudo rm -rf /tmp/rustbox

# Initialize test instance
echo "Initializing test instance..."
sudo ../target/release/rustbox init --box-id ns-test --time 60
...
sudo ../target/release/rustbox run --box-id ns-test /bin/echo -- "Hello from isolated namespace" || echo "Demo completed - namespace isolation active"
...
sudo ../target/release/rustbox run --box-id ns-test /bin/ip -- link show 2>/dev/null || echo "Network namespace isolation active"
...
sudo ../target/release/rustbox run --box-id ns-test /bin/mount 2>/dev/null || echo "Mount namespace isolation active"
...
sudo ../target/release/rustbox run --box-id ns-test --no-pid-namespace --no-mount-namespace --no-network-namespace /bin/echo -- "Hello without namespace isolation" || echo "Comparison completed"
...
echo ""
echo "=== Cleanup ==="
sudo ../target/release/rustbox cleanup --box-id ns-test
