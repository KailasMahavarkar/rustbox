...
impl ProcessCleanupManager {
    ...
    /// Register file descriptors for cleanup
    pub fn register_pipes(&self, error_pipes: Vec<RawFd>, status_pipes: Vec<RawFd>) {
        let mut pipes = self.pipes.borrow_mut();
        pipes.0 = error_pipes;
        pipes.1 = status_pipes;
    }
    ...
    pub fn emergency_cleanup(&self, reason: &str) -> Result<()> {
        ...
        eprintln!("Emergency cleanup triggered: {}", reason);        self.cleanup_file_descriptors();
    ...
    /// Count total number of processes being managed
    ...
    pub fn mark_process_exited(&self, pid: Pid) {
...
impl ProcessGuard {
    pub fn new(cleanup_manager: Arc<ProcessCleanupManager>, timeout: Duration) -> Self {
        Self {
            cleanup_manager,
            timeout,
        }
    ...
    /// Manually trigger cleanup (useful for controlled shutdown)
    pub fn cleanup_now(&self) -> Result<()> {
        self.cleanup_manager.graceful_cleanup(self.timeout)
    }
}
