...
fn test_execute_with_overrides_cpu_limit() {
    let mut isolate =
        crate::create_test_isolate("test_cpu_override").expect("Failed to create isolate");
    ...
    assert!(
        result.is_ok(),
        "execute_with_overrides should work with CPU override"
    );
...
fn test_execute_with_overrides_memory_limit() {
    let mut isolate =
        crate::create_test_isolate("test_mem_override").expect("Failed to create isolate");
    ...
    assert!(
        result.is_ok(),
        "execute_with_overrides should work with memory override"
    );
    let result = result.unwrap();
    assert!(result.success, "Command should succeed");
...
fn test_execute_with_overrides_time_limit() {
    let mut isolate =
        crate::create_test_isolate("test_time_override").expect("Failed to create isolate");
    ...
    assert!(
        result.is_ok(),
        "execute_with_overrides should work with time override"
    );
    let result = result.unwrap();
    assert!(result.success, "Command should succeed");
...
fn test_execute_with_overrides_all_limits() {
    let mut isolate =
        crate::create_test_isolate("test_all_override").expect("Failed to create isolate");
    ...
    assert!(
        result.is_ok(),
        "execute_with_overrides should work with all overrides"
    );
    let result = result.unwrap();
    assert!(result.success, "Command should succeed");
...
fn test_execute_file_with_overrides() {
    ...
    let mut isolate =
        crate::create_test_isolate("test_file_override").expect("Failed to create isolate");
...
"#;
    ...
    assert!(result.success, "Command should succeed");
    assert!(
        result.stdout.contains("Hello from shell script"),
...
fn test_original_execute_still_works() {
    let mut isolate =
        crate::create_test_isolate("test_original").expect("Failed to create isolate");
    ...
    assert!(result.success, "Command should succeed");
    assert!(
        result.stdout.contains("original method"),
        "Should contain expected output"
    );
