...
if sudo "$MINI_ISOLATE" init --box-id "42" >/dev/null 2>&1 && \
   sudo "$MINI_ISOLATE" init --box-id "43" >/dev/null 2>&1; then
    ...
    (
        sudo "$MINI_ISOLATE" run --box-id "42" --max-cpu 10 --max-time 20 \
        ...
        if result=$(sudo "$MINI_ISOLATE" run --box-id "42" --max-cpu 5 --max-time 5 \
...
if sudo "$MINI_ISOLATE" init --box-id "100" >/dev/null 2>&1 && \
   sudo "$MINI_ISOLATE" init --box-id "101" >/dev/null 2>&1; then
    ...
    (
        result1=$(sudo "$MINI_ISOLATE" run --box-id "100" --max-cpu 5 --max-time 10 \
                  /bin/echo -- "Process in box 100" 2>&1)
        if [[ "$result1" == *"Status: Success"* ]] && [[ "$result1" == *"Exit code: Some(0)"* ]]; then
    ...
    (
        result2=$(sudo "$MINI_ISOLATE" run --box-id "101" --max-cpu 5 --max-time 10 \
                  /bin/echo -- "Process in box 101" 2>&1)
        if [[ "$result2" == *"Status: Success"* ]] && [[ "$result2" == *"Exit code: Some(0)"* ]]; then
...
for attempt in {1..5}; do
    if (
        ...
        sudo "$MINI_ISOLATE" init --box-id "race_test_$attempt" >/dev/null 2>&1 &
        PID1=$!
        sudo "$MINI_ISOLATE" init --box-id "race_test_$attempt" >/dev/null 2>&1 &
        PID2=$!
    ...
    # Cleanup
    sudo "$MINI_ISOLATE" cleanup --box-id "race_test_$attempt" >/dev/null 2>&1 || true
done
...
if sudo "$MINI_ISOLATE" init --box-id "cleanup_test" >/dev/null 2>&1; then
    # Start a process and kill it mid-execution
    (
        sudo "$MINI_ISOLATE" run --box-id "cleanup_test" --max-cpu 30 --max-time 30 \
             /bin/sleep -- 25 >/dev/null 2>&1 &
        RUN_PID=$!
        sleep 2
    ...
    if result=$(sudo "$MINI_ISOLATE" run --box-id "cleanup_test" --max-cpu 5 --max-time 5 \
                /bin/echo -- "Cleanup test passed" 2>&1); then
    ...
    sudo "$MINI_ISOLATE" cleanup --box-id "cleanup_test" >/dev/null 2>&1 || true
else
...
if sudo "$MINI_ISOLATE" init --box-id "ownership_test" >/dev/null 2>&1; then
    current_uid=$(id -u)
    if result=$(sudo "$MINI_ISOLATE" run --box-id "ownership_test" --max-cpu 5 --max-time 5 \
                /bin/id -- -u 2>&1); then
    ...
    sudo "$MINI_ISOLATE" cleanup --box-id "ownership_test" >/dev/null 2>&1 || true
else
...
if sudo "$MINI_ISOLATE" init --box-id "format_test" >/dev/null 2>&1; then
    ...
    sudo "$MINI_ISOLATE" cleanup --box-id "format_test" >/dev/null 2>&1 || true
else
